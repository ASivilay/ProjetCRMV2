...
/**
 * Servlet implementation class PanierJsonServlet
 */
@WebServlet("/panierjson")
public class PanierJsonServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (idPanier != null) {
				response = new ServicePanier().find(Long.parseLong(idPanier));
			} else {
				response = new ServicePanier().list();
			}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			new ServicePanier().create(Utils.getJsonFromBuffer(req));
	...
	protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			new ServicePanier().update(Utils.getJsonFromBuffer(req));
		} catch (JsonSyntaxException e) {
			response = "Erreur : Le format des données n'est pas bon, veuillez utiliser du JSON.";
			responseStatus = 400;
		} catch (ServiceException e) {
	...
	protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			String idPanier = req.getParameter("id");
			new ServicePanier().delete(Long.parseLong(idPanier));
		} catch (NumberFormatException e) {
			response = "Le paramètre id n'est pas bon.";
			responseStatus = 400;
		} catch (ServiceException e) {
